...
setup() {
	EXPECTED_USAGE="Usage: ./testflight_post.sh [-h] [-v] [-d] [-a] [-n] [-c custCurl] [-w custUrl] [-u uploadToken] [-t teamToken] [-z dSYMfile] [datafile] [notes]"
}

@test "Check that the testflight_post.sh client is available" {
    command -v testflight_post.sh
}

@test "Script Should Output Expected Requirements" {
	run testflight_post.sh
...
@test "Script Should Warn If Release Notes Are Missing" {
	run testflight_post.sh test/fake.ipa
...
@test "Script Should Warn If Api Token Is Missing" {
	run testflight_post.sh test/fake.ipa "fake notes"
...
@test "Script Should Warn If Team Token Is Missings" {
	run testflight_post.sh -u "fakeUploadToken" test/fake.ipa "fake notes"
...
@test "Script Should Attempt Upload If All Needed Argument Are Used" {
	run testflight_post.sh -u "fakeUploadToken" -t "fakeTeamToken" test/fake.ipa "fake notes"
	
	[ "$status" -eq 0 ]
}

@test "Script Should Default To Curl" {
	run testflight_post.sh -d -u "fakeUploadToken" -t "fakeTeamToken" test/fake.ipa "fake notes"
...
@test "C Flag Should Set Curl Alternative Command" {
	EXPECTED="furl"
	run testflight_post.sh -d -c "${EXPECTED}" -u "fakeUploadToken" -t "fakeTeamToken" test/fake.ipa "fake notes"
...
@test "W Flag Should Set Url" {
	EXPECTED="http::/foo/"
 	run testflight_post.sh -d -w "${EXPECTED}" -u "fakeUploadToken" -t "fakeTeamToken" test/fake.ipa "fake notes"

	[[ ${lines[0]} =~ "${EXPECTED}" ]]
}

@test "Script Should Output Expected Version" {
	run testflight_post.sh -v

	[ "$status" -eq 1 ]
	[[ ${lines[0]} =~ "testflight_post.sh version " ]]
	[[ ${lines[0]} =~ "1.1" ]]
}
...
@test "Script Should Set File Correctly" {
	...
	run testflight_post.sh -d -u "fakeUploadToken" -t "fakeTeamToken" "$DATA_PATH" "fake notes"
...
@test "Script Should Set Upload Token Correctly" {
	...
	run testflight_post.sh -d -u "$TOKEN" -t "fakeTeamToken" test/fake.ipa "fake notes"
...
@test "Script Should Set Notes Correctly" {
	...
	run testflight_post.sh -d -u "fakeUploadToken" -t "fakeTeamToken" test/fake.ipa "${NOTES}"
...
@test "Z Flag Should Add Dsym" {
	run testflight_post.sh -d -u "fakeUploadToken" -t "fakeTeamToken" -z test/fake.dSYM test/fake.ipa "fake notes"
...
@test "Script Should Set Dsym Correctly" {
	...
	run testflight_post.sh -d -n -u "fakeUploadToken" -t "fakeTeamToken" -z "$DSYM_PATH" test/fake.ipa "fake notes"

	[[ ${lines[0]} =~ "dsym=@$DSYM_PATH" ]]
}

@test "N Flag Should Set Notify Correcty" {
	run testflight_post.sh -d -n -u "fakeUploadToken" -t "fakeTeamToken" -z test/fake.dSYM test/fake.ipa "fake notes"

	[[ ${lines[0]} =~ "notify=True" ]]
}

@test "H Flag Should Output Help" {	
	run testflight_post.sh -h

	[ "$status" -eq 0 ]
	[ "${lines[0]}" = "Usage: testflight_post.sh [-h] [-v] [-d] [-a] [-n] [-c custCurl] [-w custUrl] [-u uploadToken] [-t teamToken] [-z dSYMfile] [datafile] [notes]" ]
	[ "${lines[1]}" = "Options" ]
